...
Fire=readtable('TestSet2_fire.csv');
...
filelist=dir(fullfile('Meteorological data set1\*201503*.xls'));
n=length(filelist);
qiwen=zeros(n,1);
La_Xls=zeros(n,1);
Lo_Xls=zeros(n,1);
for i=1:n
    ...
    f=table2array(f);
    La_Xls(i,1)=str2double(cell2mat((f(22,12))));
    Lo_Xls(i,1)=str2double(cell2mat((f(22,13))));     
    qiwen(i,1)=(str2double(cell2mat((f(22,7))))-32)*5/9;
end
...
QiWen=F(Latitude,Lotitude);
 ...
 %%Precipitation
filelist=dir(fullfile('Precipitation data set1\*201503*.xls'));
n=length(filelist);
jiangshui=zeros(n,7);
La_Xls=zeros(n,1);
Lo_Xls=zeros(n,1);
...
for i=1:n
    ...
    La_Xls(i,1)=str2double(cell2mat((f(22,12))));
    Lo_Xls(i,1)=str2double(cell2mat((f(22,13))));   
    Td=(str2double(cell2mat((f(22,17))))-32)*5/9;
    T=(str2double(cell2mat((f(22,7))))-32)*5/9;
...
for i=1:n
    ...
    La_Xls(i,1)=str2double(cell2mat((f(22,12))));
    Lo_Xls(i,1)=str2double(cell2mat((f(22,13))));   
    fengsu(i,1)=str2double(cell2mat((f(22,5))));
end
...
%%Altitude
[P,R]=geotiffread('Test2_Altitude.tif');

P=imresize(P,[2081 1793]);
LaB=R.LatitudeLimits(1);
LaE=R.LatitudeLimits(2);
LoB=R.LongitudeLimits(1);
LoE=R.LongitudeLimits(2);
La=LaB:R.CellExtentInLatitude*10:LaE-R.CellExtentInLatitude;
La=La';
Dem_Latitude=repelem(La,1,1793);
Lo=LoB:R.CellExtentInLongitude*10:LoE-R.CellExtentInLongitude;
Dem_Longitude=repelem(Lo,2081,1);

DEM = griddata(Dem_Latitude, Dem_Longitude, double(P), Latitude, Lotitude);

%%Slope
[S,R]=geotiffread('Test2_Slope.tif');
S=imresize(S,[2081 1793]);
LaB=R.LatitudeLimits(1);
LaE=R.LatitudeLimits(2);
LoB=R.LongitudeLimits(1);
LoE=R.LongitudeLimits(2);
La=LaB:R.CellExtentInLatitude*10:LaE-R.CellExtentInLatitude;
La=La';
Slope_Latitude=repelem(La,1,1793);
Lo=LoB:R.CellExtentInLongitude*10:LoE-R.CellExtentInLongitude;
Slope_Longitude=repelem(Lo,2081,1);
...
test=[RH(:),QiWen(:),NDVI(:),FengSu(:),JiangShui1(:),JiangShui2(:),JiangShui3(:),JiangShui4(:),JiangShui5(:),JiangShui6(:),JiangShui7(:),DEM(:)];
