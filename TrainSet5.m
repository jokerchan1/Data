...
Fire=readtable('TrainSet5_fire.csv');
...
%%Temperature
filelist=dir(fullfile('Meteorological data set2\*201203*.xls'));
n=length(filelist);
qiwen=zeros(n,1);
La_Xls=zeros(n,1);
Lo_Xls=zeros(n,1);
for i=1:n
    ...
    f=table2array(f);
    La_Xls(i,1)=str2double(cell2mat((f(13,12))));
    Lo_Xls(i,1)=str2double(cell2mat((f(13,13))));     
    qiwen(i,1)=(str2double(cell2mat((f(13,7))))-32)*5/9;
end
F=scatteredInterpolant(La_Xls,Lo_Xls,qiwen,'natural','nearest');
QiWen=F(Latitude,Lotitude);

 %%Precipitation
filelist=dir(fullfile('Precipitation data set2\*201203*.xls'));
n=length(filelist);
jiangshui=zeros(n,7);
La_Xls=zeros(n,1);
Lo_Xls=zeros(n,1);
...
for i=1:n
    ...
    La_Xls(i,1)=str2double(cell2mat((f(13,12))));
    Lo_Xls(i,1)=str2double(cell2mat((f(13,13))));   
    Td=(str2double(cell2mat((f(13,17))))-32)*5/9;
    T=(str2double(cell2mat((f(13,7))))-32)*5/9;
...
for i=1:n
    ...
    La_Xls(i,1)=str2double(cell2mat((f(13,12))));
    Lo_Xls(i,1)=str2double(cell2mat((f(13,13))));   
    fengsu(i,1)=str2double(cell2mat((f(13,5))));
end
...
[P,R]=geotiffread('Train5_Altitude.tif');
p=imresize(P,[713,772]);
...
La=LaB:R.CellExtentInLatitude*10:LaE-R.CellExtentInLatitude;
La=La';
Slope_Latitude=repelem(La,1,772);
Lo=LoB:R.CellExtentInLongitude*10:LoE-R.CellExtentInLongitude;
Slope_Longitude=repelem(Lo,713,1);
