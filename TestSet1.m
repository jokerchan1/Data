...
Fire=readtable('TestSet1_fire.csv');
...
filelist=dir(fullfile('Meteorological data set1\*201803*.xls'));
n=length(filelist);
qiwen=zeros(n,1);
La_Xls=zeros(n,1);
Lo_Xls=zeros(n,1);
for i=1:n
    ...
    f=table2array(f);
    La_Xls(i,1)=str2double(cell2mat((f(30,12))));
    Lo_Xls(i,1)=str2double(cell2mat((f(30,13))));     
    qiwen(i,1)=(str2double(cell2mat((f(30,7))))-32)*5/9;
end
F=scatteredInterpolant(La_Xls,Lo_Xls,qiwen,'natural','nearest');
QiWen=F(Latitude,Lotitude);

filelist=dir(fullfile('Precipitation data set1\*201803*.xls'));
n=length(filelist);
jiangshui=zeros(n,7);
La_Xls=zeros(n,1);
Lo_Xls=zeros(n,1);
...
for i=1:n
    ...
    La_Xls(i,1)=str2double(cell2mat((f(30,12))));
    Lo_Xls(i,1)=str2double(cell2mat((f(30,13))));   
    Td=(str2double(cell2mat((f(30,17))))-32)*5/9;
    T=(str2double(cell2mat((f(30,7))))-32)*5/9;
...
for i=1:n
    ...
    La_Xls(i,1)=str2double(cell2mat((f(30,12))));
    Lo_Xls(i,1)=str2double(cell2mat((f(30,13))));   
    fengsu(i,1)=str2double(cell2mat((f(30,5))));
end
...
[P,R]=geotiffread('Test1_Altitude.tif');
P=imresize(P,[2155,3499]);
...
La=LaB:R.CellExtentInLatitude*10:LaE-R.CellExtentInLatitude;
La=La';
Slope_Latitude=repelem(La,1,3499);
Lo=LoB:R.CellExtentInLongitude*10:LoE-R.CellExtentInLongitude;
Slope_Longitude=repelem(Lo,2155,1);
