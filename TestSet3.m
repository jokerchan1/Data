...
Fire=readtable('TestSet3_fire.csv');
...
filelist=dir(fullfile('Meteorological data set1\*201903*.xls'));
n=length(filelist);
qiwen=zeros(n,1);
La_Xls=zeros(n,1);
Lo_Xls=zeros(n,1);
...
QiWen=F(Latitude,Lotitude);
 ...
 %%Precipitation
filelist=dir(fullfile('Precipitation data set1\*201903*.xls'));
n=length(filelist);
jiangshui=zeros(n,7);
La_Xls=zeros(n,1);
Lo_Xls=zeros(n,1);
...
% 
%%Altitude
[P,R]=geotiffread('Test3_Alitude.tif');
P(P<0)=330;
P(P==0)=400;
P=imresize(P,[1501 2074], 'bicubic');
...
La=La';
Dem_Latitude=repelem(La,1,2074);
Lo=LoB:R.CellExtentInLongitude*10:LoE-R.CellExtentInLongitude;
Dem_Longitude=repelem(Lo,1501,1);
